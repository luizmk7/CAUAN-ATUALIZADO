...
existing_by_idx = {it['index']: it for it in classified}

missing_items = [it for it in manifest['unique'] if it['index'] not in existing_by_idx]
print(f"Processing {len(missing_items)} missing items...")
...
for item in missing_items:
    ...
    api_url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-3.5-flash-lite:generateContent?key={api_key}"
    ...
    if not success:
        print(f"[{idx:02d}] Failed after 5 attempts")

print("Done processing missing items.")
