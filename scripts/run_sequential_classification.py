...
classified_cache = {}
if os.path.exists('/tmp/cauan_images/classified_items.json'):
    try:
        with open('/tmp/cauan_images/classified_items.json') as f:
            for it in json.load(f):
                if it.get('confidence', 0) >= 0.8:
                    classified_cache[it['index']] = it
    except Exception:
        pass

print(f"Loaded {len(classified_cache)} already classified images.")
...
for item in unique_items:
    idx = item['index']
    if idx in classified_cache:
        all_results.append(classified_cache[idx])
        continue
            ...
            try:
                ...
                if 'error' in data:
                    ...
                    if code == 429:
                        ...
                        # print(f"[{idx:02d}] 429 on {model}, waiting {wait_time}s...")
                ...
                print(f"[{idx:02d}] {result['category'].upper()}: {result['title']} - {result['visual_description'][:50]}...")
                all_results.append(result)
                classified_cache[idx] = result
    ...
    if not success:
        print(f"[{idx:02d}] Fallback to eventos")
        fallback = {
            'id': f"img-{idx:02d}",
            'index': idx,
            'url': item['url'],
            'category': 'eventos',
            'confidence': 0.7,
            'visual_description': 'Registro de celebração',
            'title': 'Celebração e Momentos',
            'kicker': 'Eventos',
