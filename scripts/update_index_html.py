...
def build_section_content(items_list, cat_slug, cat_display, is_dark):
...
{chr(10).join(cards)}
      ...
      <!-- Indicadores de carrossel {cat_display} -->
      <div class="carousel-dots" role="tablist" aria-label="Navegar pelas fotos de {cat_display}">
{chr(10).join(dots)}
      </div>'''
    return grid_block

form_grid_block = build_section_content(form_items, "formaturas", "Formatura", is_dark=False)
ev_grid_block = build_section_content(ev_items, "eventos", "Evento", is_dark=True)
cas_grid_block = build_section_content(cas_items, "casamentos", "Casamento", is_dark=True)
...
if not m_form:
    ...
    html = html[:m_form.start(2)] + form_grid_block + html[m_form.end(2):]
    print("SUCCESS: Replaced Formaturas block")
...
if not m_ev:
    ...
    html = html[:m_ev.start(2)] + ev_grid_block + html[m_ev.end(2):]
    print("SUCCESS: Replaced Eventos block")
...
if not m_cas:
    ...
    html = html[:m_cas.start(2)] + cas_grid_block + html[m_cas.end(2):]
    print("SUCCESS: Replaced Casamentos block")

# Ensure script tag is present
if 'data/portfolio-manifest.js' not in html:
    html = html.replace('<script src="app.js"></script>', '<script src="data/portfolio-manifest.js"></script>\n  <script src="app.js"></script>')
    print("SUCCESS: Added portfolio-manifest.js script tag")
